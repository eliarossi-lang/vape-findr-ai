...
const cardData = [
  { color: '#120F17', title: 'Ricerca AI', description: 'Trova il vape ideale parlando come pensi.', label: 'Smart Search' },
  { color: '#120F17', title: 'Catalogo', description: 'Liquidi, pod, mod e accessori sempre aggiornati.', label: 'Prodotti' },
  { color: '#120F17', title: 'Mappa Negozi', description: 'Shop fisici verificati vicino a te.', label: 'Local' },
  { color: '#120F17', title: 'Alert Prezzi', description: 'Notifiche quando il prezzo scende.', label: 'Risparmio' },
  { color: '#120F17', title: 'Recensioni', description: 'Opinioni verificate dalla community.', label: 'Trust' },
  { color: '#120F17', title: 'Sicurezza', description: 'Solo prodotti conformi e tracciati.', label: 'Safety' },
];
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e) => {
      ...
      cards.forEach((card) => {
        ...
        const cxr = cr.left + cr.width / 2; const cyr = cr.top + cr.height / 2;
        const distance = Math.hypot(e.clientX - cxr, e.clientY - cyr) - Math.max(cr.width, cr.height) / 2;
    ...
    <div className="bento-section" style={{ '--glow-color': glowColor }}>
      ...
      <div className="card-grid" ref={gridRef}>
        {cardData.map((card, index) => {
          ...
          return (
            <div key={index} className={baseClassName} style={{ backgroundColor: card.color, '--glow-color': glowColor }}>
              {inner}
            </div>
          );
        })}
